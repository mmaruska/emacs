...
/* mmc:*/
void
x_fill_frame_area_bg (f, x, y, width, height) /* , exposures */
     struct frame *f;
     int x, y;
     int width, height;
     /*     int exposures;*/
{
  xassert (width > 0 && height > 0);
#if 0
  fprintf(stderr, "%sXClearArea%s\n", color_green, color_reset);
  XClearArea (dpy, window, x, y, width, height, exposures);
#else
  /* mmc: */
  if (f && (f->output_data.x) && f->output_data.x->reverse_gc)
    XFillRectangle (FRAME_X_DISPLAY (f), FRAME_X_WINDOW (f), f->output_data.x->reverse_gc, x, y, width, height);
  else {/* fixme!  */
    fprintf(stderr, "%s: no frame given, or the frame does not have reverse_gc -> XClearArea cannot be emulated\n", __FUNCTION__);
    /* exit (-1); */
  }
#endif
}
...
#if 0 /* mmc!       is this enough? i think the core is in invalidate_window_matrices ... in dispnew.c*/
              SET_FRAME_GARBAGED (f);
#else
#if MMC_DEBUG
